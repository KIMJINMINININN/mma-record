...
### 4c. 마이그레이션/시드 확인
- 원격에 0001~0018 적용 + **프리셋(기술) 시드 라이브**여야 한다(아래 §6 검증의 "프리셋" 항목).
- **0017(technique level) · 0018(즐겨찾기)은 컬럼 추가** — 코드보다 **먼저** 적용해야 한다(아래 §5-0 선적용 단계).
...
[ ] Supabase: private 버킷(training-media) + 0001~0018 + 프리셋 시드 (0017·0018=컬럼 추가 → 코드보다 먼저)
