...
import { createPushHandlers } from './handlers/push-handlers';

export function useWebviewMessage(ctx: HandlerContext) {
  // Compose dispatch table: common → auth → media → reminder → push → system (system wins on key conflict)
