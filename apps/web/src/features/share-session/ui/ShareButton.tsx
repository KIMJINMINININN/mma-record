...
/**
 * ShareButton — 공유 링크 복사 진입점 클라 아일랜드 (F11 / 0022_shares.sql · 0024_share_technique.sql).
 *
 * 세션 카드(서버 컴포넌트) · 기술 상세가 공통으로 쓰는 **제네릭** 버튼이다. SessionFavoriteStar와 동일하게
 * **feature 안**에 작은 클라 아일랜드로 두고, 서버 액션(createShare)을 **직접** import 한다(배럴로 재노출하면
 * 위젯/서버 그래프에 server-only가 누수돼 테스트가 깨진다 — session-favorite 관용구). 그래서 배럴은
 * 자원별 얇은 래퍼(SessionShareButton/TechniqueShareButton)만 내보내고, 이 제네릭/createShare는 숨긴다.
 *
 * 클릭 시 createShare(resourceType, resourceId)로 공유 토큰을 만들거나 재사용(멱등)하고, 성공하면
...
export function ShareButton({
  ...
  const onShare = () => {
    startTransition(async () => {
      const res = await createShare(resourceType, resourceId);
      if (res.ok) {
